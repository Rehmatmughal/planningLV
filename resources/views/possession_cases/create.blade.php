            ...
            <div class="card-body">
                            ...
                            <select id="property_type_id"
                                ...
                                @foreach($propertyTypes as $propertyType)
                                    ...
                                    <option value="{{ $propertyType->id }}"
                                        {{ old('property_type_id', $selectedPlot?->property_type_id) == $propertyType->id ? 'selected' : '' }}>

                                        {{ $propertyType->name ?? $propertyType->title }}
                        ...
                        <div>
                            ...
                            <div id="selectedPlotText"
                                ...
                                @if($selectedPlot)
                                    ...
                                    {{-- test for area from area variation --}}
                                    @if($selectedPlot->latestAreavariation)
                                        - {{ $selectedPlot->latestAreavariation->measured_area ?? $selectedPlot->latestAreavariation->measured_area ?? '-' }}
                                    @endif
                ...
                <div class="row g-3">
                    ...
                    <div class="col-md-3">
                        ...
                        <small class="text-muted">
                            Automatically determined from previous possession records.
                        </small>
                    ...
                    {{-- <div class="col-md-3">
                        ...
                        <input type="text"
                            name="possession_no"
                            class="form-control @error('possession_no') is-invalid @enderror"
                            value="{{ old('possession_no') }}"
                        ...
                        <small class="text-muted">
                            Normally auto-generated by the system.
                        </small>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    */
    ...
    const possessionNoInput =
        document.getElementById('possession_no');

    const caseTypeInput =
        document.getElementById('case_type');


    // const oldBlockId = @json($selectedPlot?->block_id);
    // const oldStreetId = @json($selectedPlot?->street_id);
    const oldPropertyTypeId =
        @json($selectedPlot?->property_type_id);

    const oldBlockId =
        @json($selectedPlot?->block_id);

    const oldStreetId =
        @json($selectedPlot?->street_id);
    ...
    */
    ...
    function loadBlocks(
        ...
        fetch(
            ...
            projectId +
            '?property_type_id=' +
            encodeURIComponent(propertyTypeId)
        )
    ...
    */
    ...
    // test for search button auto select on enter
    plotNumberInput.addEventListener('keydown', function (event) {

        if (event.key === 'Enter') {

            event.preventDefault();

            if (!searchPlotBtn.disabled) {
                searchPlotBtn.click();
            }
        }
    ...
    */
    ...
    plotResults.addEventListener('click', function (event) {
        ...
        /*
        |--------------------------------------------------------------------------
        | Automatically Load Next Possession Number
        |--------------------------------------------------------------------------
        */

        possessionNoInput.value = 'Loading...';
        caseTypeInput.value = 'Loading...';


        fetch(
            '{{ url("possession-cases/ajax/possession-preview") }}/' +
            plotId
        )
        ...
        .then(response => {
            ...
            if (!response.ok) {
                throw new Error(
                    'Unable to load possession number.'
                );
            }
        ...
        .catch(error => {
            ...
            plotSearchMessage.innerHTML = `
                <div class="alert alert-danger">
                    Plot selected, but possession number
